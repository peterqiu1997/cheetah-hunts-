;PROBLEM WITH SWITCHING TARGETS? SEE BELOW
;ACQUIRE TARGET ISSUES WITH TWO CHEETAHS >> FIXED USE AVERAGE DISTANCE?
;HOW MUCH VARIATION IN SPEED RANDOMIZATION 
;IGNORE CHEETAH IF ALREADY CAUGHT. 
;HOW MUCH MEAT PER CHEETAH??
...
globals [
  ...
  COMx                  
  COMy 
  hunted  ;used for behaviorspace output        
...
to setup
  ...
  ifelse kpfactor <= 8 [set kpfactor true][set kpfactor false]
  ...
  create-cheetahs 3 [
    set color red
    set mass 50 + random(20) - 10 ;change if based on one cheetah or not
    set size 2
    set caught-gazelle? false
    set vcheetah vprowl
    set maxvcheetah 24
    ask cheetah 0 [
      set xcor rspawn * (cos(init-angle0))
      set ycor rspawn * (sin(init-angle0))
    ]
    ask cheetah 1 [
      set xcor rspawn * (cos(init-angle1))
      set ycor rspawn * (sin(init-angle1))
    ]
    ask cheetah 2 [
      set xcor (rspawn) * (cos(init-angle2))
      set ycor (rspawn) * (sin(init-angle2))
    ]
  ]  
  ...
  create-gazelles herdsize [ 
    set mass 71 + random(24) - 12
    set maxvgazelle 16 + random(2) - 1
    set energy-possessed 200 + random(50) - 25
    set vgazelle basevgazelle
    set color white
    set size 2
...
to go
  every timescale(1) [ 
    ...
    ask cheetahs [  
      ...
      acquiretarget                  ;previously in ask-concurrent
      move-to-prey
      updatetarget   
      checkcaught
    ]
    ...
    ask gazelles [ 
      ...
      if (chase? or detected?) [                                                                                                   
        ...
        [disperse] 
        ifelse any? cheetahs with [caught-gazelle? = true] 
        [ask targetgazelle [decelerategazelle (0.25)]]   ;MIGHT have to change this 0.5 to something more plausible
        [accelerategazelle (4)]
      ]
    ...
    if (caught?) [
      ...
      print (word "Hunt successful! Total energy change: " total-energy " kcal." )
      print (word "Energy change for each cheetah: " (total-energy / 3) " kcal.")
    ...
    if (ticks > chaseduration) [ 
      set hunted 0 
      ask cheetahs [total-energy-expended]
      print (word "Hunt failed. Total energy change: " total-energy " kcal." )
      print (word "Energy change for each cheetah: " (total-energy / 3) " kcal.")
      print (word "Distance travelled: " dist?)
      stop ]  
...
;________________________________________________________________________________________Energetics_____________________________________________________________________________________________
...
to-report dist?
  set avg-dist-trav ([distance-covered] of cheetah 0 + [distance-covered] of cheetah 1 + [distance-covered] of cheetah 2) / 3
...
to-report look-energy
  report 5.8 * (mass ^ 0.75) * (look-time) + 2.6 * (mass ^ 0.6) * 2
end
...
to move-to-prey
  ...
  [
    ifelse any? cheetahs with [caught-gazelle? = true]
    [set heading towards gazelle targetnumber]
  ...
  ifelse chase? [fd timescale(vcheetah)] [fd timescale(vprowl)]
...
to acquiretarget     
  if( (not(acquired?) and (distancexy COMx COMy < attackdistance or detected?) ) ) 
    [                                                                         
      set targetgazelle closest-to-coalition gazelles                                                                
      set targetnumber [who] of targetgazelle
      set mindist (distance targetgazelle)
      set acquired? true
    ]
  if (switch? and not(caught-gazelle?)) [
    if (acquired? and ticks < 1000 and ticks > 300 and not(already-switched?)) [
      ask gazelles in-cone 10 210
...
to checkcaught 
  if (acquired?) [
    ask cheetahs [ if(distance targetgazelle < rcaught) [set caught-gazelle? true set vcheetah [vgazelle] of targetgazelle] ]
    if all? cheetahs [ caught-gazelle? = true ]
    [set caught? true]
  ]
end
...
to-report pursuer
  ifelse([caught-gazelle?] of cheetah 0 or [caught-gazelle?] of cheetah 0) 
  [
    if([caught-gazelle?] of cheetah 0)
    [report cheetah 1]
    if([caught-gazelle?] of cheetah 1)
    [report cheetah 0]
  ]
  [
...
to adjustvelocity     
  ...
  [
    ...
    ;if vcheetah < vprowl [set (vcheetah) (vprowl)]
...
to-report calccheetahmaxturn
  ifelse(caught-gazelle?) 
  [
    if(who = 0) [set vcheetah [vgazelle] of targetgazelle]
    if(who = 1) [set vcheetah [vgazelle] of targetgazelle]
    report 90
  ]
...
to-report avgdist [thing1 thing2 thing3]      ;using average distance to acquire the target
  report ((distance thing1) + (distance thing2) + (distance thing3)) / 2
end

to-report closest-to-coalition [things]
  report min-one-of things [avgdist (cheetah 0) (cheetah 1) (cheetah 2)] 
end
...
"pen-1" 1.0 0 -13840069 true "" "if (ticks mod 10 = 0) [ask cheetah 1 [plot vcheetah]]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if (ticks mod 10 = 0) [plot [cheetahmaxturn] of cheetah 0]"
"pen-1" 1.0 0 -13840069 true "" "if (ticks mod 10 = 0) [plot [cheetahmaxturn] of cheetah 1]"
...
@#$#@#$#@
<experiments>
  <experiment name="Three cheetahs friction" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>hunt?</metric>
  ...
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="init-angle2">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="minimum-separation">
      <value value="1.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="look-time">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="frictioncoefficient">
      <value value="1.3"/>
    ...
    <enumeratedValueSet variable="init-angle0">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="positioning-time">
      <value value="1.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="init-angle1">
      <value value="75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="vprowl">
      <value value="7"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="gazellemaxturn">
      <value value="2.1"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="combined" repetitions="300" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>hunt?</metric>
    <metric>total-energy?</metric>
