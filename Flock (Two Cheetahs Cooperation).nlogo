...
to setup
  ...
  ifelse kpfactor <= 10 [set kpfactor true][set kpfactor false] ;slightly less kpfactor based off of larger # cheetahs
  ...
  create-cheetahs 2 [
    set mass 40 + random(10) - 5 ;change if based on one cheetah or not
    set color red
    set size 2
  ...
  ask cheetahs [pendown]
  
  create-gazelles herdsize [ 
    set mass 60 + random(10) - 5
...
to go
  every timescale(1) [ 
    ...
    if (caught?) [
      ...
      print (word "Energy change per cheetah: " (total-energy / 2) " kcal.")
...
;_________________________________________________________________________________Energetics____________________________________________________________________________________________________
to-report hunt?
  report hunted
end
...
to-report dist?
  set avg-dist-trav ([distance-covered] of cheetah 0 + [distance-covered] of cheetah 1) / 2
...
to-report look-energy
  report 5.8 * (mass ^ 0.75) * (look-time) + 2.6 * (mass ^ 0.6) * (positioning-time * 2)
end
...
to acquiretarget   
  ...
  if (switch?) [
    if (acquired? and ticks < 1000 and ticks > 300 and not(already-switched?)) [
      ask gazelles in-cone 10 210 ;cheetah field of vision is 210 degrees radius is decreased tremendously >> coalitions much more focused?
...
to-report avgdist [thing1 thing2]      ;using average distance to acquire the target
  report ((distance thing1) + (distance thing2)) / 2
end

to-report closest-to-coalition [things]
  report min-one-of things [avgdist (cheetah 0) (cheetah 1)] 
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot [cheetahmaxturn] of cheetah 0"
"pen-1" 1.0 0 -13840069 true "" "plot [cheetahmaxturn] of cheetah 1"
...
<experiments>
  ...
  <experiment name="friction difference" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>hunt?</metric>
    <metric>total-energy?</metric>
  ...
  <experiment name="positioning" repetitions="80" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>hunt?</metric>
    <metric>total-energy?</metric>
