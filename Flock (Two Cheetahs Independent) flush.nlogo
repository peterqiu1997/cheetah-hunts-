...
to setup
  ...
  ifelse kpfactor <= 10 [set kpfactor true][set kpfactor false]
  set target-list []
  ...
  create-cheetahs 2 [
    ...
    set maxvcheetah 24
    ask cheetah 0 [
      set xcor 60 * (cos(init-angle0))
      set ycor 60 * (sin(init-angle0))
      set heading (towardsxy 0 0)
    ]
    ask cheetah 1 [
      set xcor 40 * (cos(init-angle1))
      set ycor 40 * (sin(init-angle1))
      set heading (towardsxy 0 0)
    ]
    pendown
  ]  
...
to go
  every timescale(1) [ 
    ask cheetahs [  
      ...
      ifelse(who = 0 and (ticks < 650 and distance closest gazelles > 35)) [
        set cheetah0-time ticks]
      [
        set cheetah1-time ticks
        adjustvelocity  
        acquiretarget                  
        move-to-prey
        updatetarget   
    ...
    ask gazelles [ 
      ...
      check-maneuver
      adjust-gazelle-velocity
      
      ifelse (([chase?] of pursuer) or detected?) [   
        accelerategazelle (4)                                                                                                
        ifelse(maneuver? and member? (self) target-list)
        [maneuver]
        [ifelse distance pursuer < (rdetection - 3)
          [disperse] 
          [flock]
        ]
      ]
    ...
    if (caught?) [
      set cheetah0-time cheetah1-time - cheetah0-time
      print(cheetah1-time - cheetah0-time)
      set hunted 1 
      ;set avg-dist-trav total
      ask cheetahs [total-energy-expended]
      print (word "Hunt successful! Net energy change: " total-energy " kcal. " )
      print (word "Energy change for each cheetah: " (total-energy / 2) " kcal. ")
    ...
    if (ticks > chaseduration) [ 
      ...
      [
        set hunted 1
        print (word "Caught 1. Net energy change: " total-energy " kcal. ")
        print (word "Energy change for each cheetah: " (total-energy / 2) " kcal. ")
        print (word "Distance travelled: " dist?)
      ] 
      [
...
;____________________________________________________________________________________Energetics_________________________________________________________________________________________________
to-report hunt?
  report hunted
end
...
to-report cheetah0-energy?
end

to-report cheetah1-energy?
end
...
to-report hunting-energy 
  if(who = 0)
  [report 78.3 * (mass ^ 0.84) * (to-hour (cheetah0-time) + 15 / 3600)]
  if(who = 1)
  [report 78.3 * (mass ^ 0.84) * (to-hour (cheetah1-time) + 15 / 3600)] ;aldama/calder equations
end
...
to total-energy-expended
  ...
  ifelse (not(kpfactor)) ;kleptoparasitism factor
...
to move-to-prey
  ifelse(acquired?)
  [
    turn-towards (towards targetgazelle) cheetahmaxturn
  ask targetgazelle [ set color 85 pendown]
  set chase? true
  ask gazelles [set is-chasing? true]
  ]
  [
    findCOM  
...
to acquiretarget   
  if( (not(acquired?) and (distancexy COMx COMy < attackdistance or detected?) ) ) 
    [                                                                         
      ...
      set target-list lput targetgazelle target-list
      set acquired? true
      ask gazelles [set targeted? true]
    ]
;    if (switch?) [
;      if (acquired? and ticks < 1000 and ticks > 300 and not(already-switched?)) [
;        ask gazelles in-cone 5 210 ;cheetah field of vision is 210 degrees radius is decreased tremendously >> coalitions much more focused?
...
to updatetarget
  if(acquired?)
  [foreach target-list [create-link-with targetgazelle]]
end

to checkcaught 
  if (acquired?) [
    foreach target-list [
      if(distance targetgazelle < rcaught) [
        set caught-gazelle? true 
        set vcheetah 0
        ask targetgazelle [ set vgazelle 0] ]
    ]
  ]
...
to adjustvelocity   
  if(acquired?)[  
    ...
    ifelse [maneuver?] of targetgazelle 
    [decelerate 4] 
    [if chase? [accelerate 3]]
...
to adjust-gazelle-velocity
  if vgazelle > maxvgazelle [set (vgazelle) (maxvgazelle)]
...
"" ""
PENS
"default" 1.0 0 -11221820 true "" "ask cheetah 0 [plot cheetahmaxturn]"
"pen-1" 1.0 0 -13840069 true "" "ask cheetah 1 [plot cheetahmaxturn]"
...
@#$#@#$#@
<experiments>
  <experiment name="Energetics Test Run for Data" repetitions="300" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>hunt?</metric>
